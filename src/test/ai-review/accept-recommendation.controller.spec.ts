...
describe('AiReviewController acceptRecommendation', () => {
  ...
  it('POST /api/v1/ai-reviews/:id/accept-recommendation with valid JWT and ACCEPT review returns 200', async () => {
    ...
    jest.spyOn(service, 'acceptRecommendation').mockResolvedValue(mockResponse as never);
    jest.spyOn(clsService, 'get').mockReturnValue('freelancer-1');

    const result = await controller.acceptRecommendation('review-1', { createChangeRequests: false });

    expect(result).toEqual(mockResponse);
    expect(service.acceptRecommendation).toHaveBeenCalledWith('review-1', { createChangeRequests: false }, 'freelancer-1');
  });
  ...
  it('POST /api/v1/ai-reviews/:id/accept-recommendation without JWT returns 401 via guard', () => {
    ...
    const guards = Reflect.getMetadata('__guards__', AiReviewController.prototype.acceptRecommendation);
    expect(guards).toBeDefined();
  });

  it('POST /api/v1/ai-reviews/:id/accept-recommendation on non-existent review returns 404', async () => {
    jest.spyOn(service, 'acceptRecommendation').mockRejectedValue({ code: 'AI_REVIEW_NOT_FOUND', status: 404 });
    jest.spyOn(clsService, 'get').mockReturnValue('freelancer-1');

    await expect(
      controller.acceptRecommendation('review-1', { createChangeRequests: false }),
  ...
  it('POST /api/v1/ai-reviews/:id/accept-recommendation on already-accepted review returns 409', async () => {
    jest.spyOn(service, 'acceptRecommendation').mockRejectedValue({ code: 'AI_REVIEW_ALREADY_COMPLETED', status: 409 });
    jest.spyOn(clsService, 'get').mockReturnValue('freelancer-1');
