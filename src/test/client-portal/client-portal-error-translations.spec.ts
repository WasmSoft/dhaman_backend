...
describe('Client Portal — Error Translations (US2)', () => {
  ...
  describe('Fallback behavior', () => {
    it('should return English message for unknown locale', () => {
      const enResult = translator.translate(
        ErrorCode.PORTAL_TOKEN_INVALID,
        Locale.EN,
      );
      const unknownResult = translator.translate(
        ErrorCode.PORTAL_TOKEN_INVALID,
        'FR' as Locale,
      );
    ...
    it('should return INTERNAL_SERVER_ERROR for unknown code', () => {
      const result = translator.translate(
        'UNKNOWN_ERROR_CODE' as ErrorCode,
        Locale.EN,
      );
      const internalError = translator.translate(
    ...
    it('should return default English message via getDefaultMessage', () => {
      const result = translator.getDefaultMessage(
        ErrorCode.PORTAL_TOKEN_INVALID,
      );
      expect(result).toBeDefined();
      expect(typeof result).toBe('string');
      expect(result).toBe(
        translator.translate(ErrorCode.PORTAL_TOKEN_INVALID, Locale.EN),
      );
    });
  });
