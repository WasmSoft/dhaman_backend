...
import { ChangeRequestResponseDto, ChangeRequestListItemDto } from './dto/change-request-response.dto';
...
const ERR_CR_NOT_FOUND = 'CHANGE_REQUEST_NOT_FOUND — Change request was not found';
const ERR_FORBIDDEN = 'FORBIDDEN — You do not have permission to access this change request';
const ERR_PORTAL_TOKEN_INVALID = 'PORTAL_TOKEN_INVALID — Client portal link is invalid';
const ERR_VALIDATION = 'VALIDATION_ERROR — Invalid request data';
const ERR_NOT_EDITABLE = 'CHANGE_REQUEST_NOT_EDITABLE — Change request cannot be edited in its current state';
const ERR_NOT_SENDABLE = 'CHANGE_REQUEST_NOT_SENDABLE — Change request cannot be sent in its current state';
const ERR_NOT_APPROVABLE = 'CHANGE_REQUEST_NOT_APPROVABLE — Change request cannot be approved in its current state';
const ERR_NOT_DECLINABLE = 'CHANGE_REQUEST_NOT_DECLINABLE — Change request cannot be declined in its current state';
const ERR_NOT_APPROVED = 'CHANGE_REQUEST_NOT_APPROVED — Change request must be approved before funding';
const ERR_AMOUNT_INVALID = 'CHANGE_REQUEST_AMOUNT_INVALID — Amount must be greater than zero';
const ERR_NOT_FUNDABLE = 'PAYMENT_NOT_FUNDABLE — Payment cannot be funded in its current state';
...
export class ChangeRequestsController {
  ...
  @ApiOperation({
    ...
    description: 'Returns paginated list of change requests scoped to the agreement owned by the authenticated freelancer.',
  })
  @ApiParam({ name: 'id', description: 'Agreement UUID' })
  @ApiQuery({ name: 'status', required: false, description: 'Filter by change request status' })
  @ApiQuery({ name: 'milestoneId', required: false, description: 'Filter by milestone UUID' })
  ...
  @ApiResponse({ status: 200, description: 'Paginated list of change requests', type: [ChangeRequestListItemDto] })
  ...
  @ApiOperation({
    ...
    description: 'Creates an out-of-scope paid change request for the agreement. Freelancer must own the agreement.',
  })
  @ApiParam({ name: 'id', description: 'Agreement UUID' })
  @ApiBody({ type: CreateChangeRequestDto })
  @ApiResponse({ status: 201, description: 'Change request created', type: ChangeRequestResponseDto })
  ...
  @ApiOperation({
    ...
    description: 'Returns the full detail of a change request with payment summary.',
  })
  @ApiParam({ name: 'id', description: 'Change request UUID' })
  @ApiResponse({ status: 200, description: 'Change request detail', type: ChangeRequestResponseDto })
  @ApiResponse({ status: 401, description: ERR_UNAUTHORIZED })
  ...
  @ApiOperation({
    ...
    description: 'Updates editable fields of a change request. Only allowed in DRAFT state before client approval.',
  })
  @ApiParam({ name: 'id', description: 'Change request UUID' })
  @ApiBody({ type: UpdateChangeRequestDto })
  @ApiResponse({ status: 200, description: 'Change request updated', type: ChangeRequestResponseDto })
  @ApiResponse({ status: 401, description: ERR_UNAUTHORIZED })
  @ApiResponse({ status: 404, description: ERR_CR_NOT_FOUND })
  ...
  @ApiParam({ name: 'id', description: 'Change request UUID' })
  @ApiResponse({ status: 200, description: 'Change request sent', type: ChangeRequestResponseDto })
  @ApiResponse({ status: 401, description: ERR_UNAUTHORIZED })
  @ApiResponse({ status: 404, description: ERR_CR_NOT_FOUND })
  ...
  @ApiOperation({
    ...
    description: 'Client approves a change request via portal token. Creates a separate waiting payment through the Payments service.',
  })
  @ApiParam({ name: 'token', description: 'Client portal access token' })
  @ApiParam({ name: 'id', description: 'Change request UUID' })
  @ApiResponse({ status: 200, description: 'Change request approved', type: ChangeRequestResponseDto })
  ...
  @ApiOperation({
    ...
    description: 'Client declines a change request with a reason. Original milestone payment remains unchanged.',
  })
  @ApiParam({ name: 'token', description: 'Client portal access token' })
  @ApiParam({ name: 'id', description: 'Change request UUID' })
  @ApiBody({ type: PortalDeclineChangeRequestDto })
  @ApiResponse({ status: 200, description: 'Change request declined', type: ChangeRequestResponseDto })
  ...
  @ApiOperation({
    ...
    description: 'Client funds the separate change request payment in demo mode. Change request must be approved before funding. Original milestone payment remains unchanged.',
  })
  @ApiParam({ name: 'token', description: 'Client portal access token' })
  @ApiParam({ name: 'id', description: 'Change request UUID' })
  @ApiBody({ type: PortalFundPaymentDto })
  @ApiResponse({ status: 200, description: 'Payment reserved and change request funded', type: ChangeRequestResponseDto })
  @ApiResponse({ status: 400, description: ERR_VALIDATION })
  @ApiResponse({ status: 401, description: ERR_PORTAL_TOKEN_INVALID })
  @ApiResponse({ status: 404, description: ERR_CR_NOT_FOUND })
