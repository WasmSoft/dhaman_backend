...
export class PaymentSummaryDto {
  ...
  @ApiProperty({ description: 'Amount in Decimal-safe string format', example: '500.00' })
  amount!: string;

  @ApiProperty({ description: 'Currency code', example: 'USD' })
  currency!: string;

  @ApiProperty({ enum: PaymentOperationType, description: 'Payment operation type' })
...
export class ChangeRequestResponseDto {
  ...
  @ApiPropertyOptional({ description: 'AI review UUID that triggered this request' })
  aiReviewId!: string | null;

  @ApiProperty({ enum: TimelineActorRole, description: 'Role that requested this change' })
  ...
  @ApiProperty({ description: 'Extra work price as Decimal-safe string', example: '500.00' })
  amount!: string;

  @ApiProperty({ description: 'ISO 4217 currency code (uppercase)', example: 'USD' })
  currency!: string;
  ...
  @ApiProperty({ enum: ChangeRequestStatus, description: 'Change request status' })
